...
fp = open(timestr + '_VortexCopt2ndOrder.csv', 'w')
...
def axis_accel_APF(distance_ab, px_a, py_a, px_b, py_b, vx_a, vy_a, vx_b, vy_b):
        ...
        ax_APF = 4.2 * (-1/safety_radius+1/distance_ab) * (px_a - px_b) / distance_ab**3 - 11*(-1/2+1/v_rel)*(vx_a - vx_b) / v_rel - 55.2*(vy_a - vy_b) / v_rel
        ay_APF = 4.2 * (-1/safety_radius+1/distance_ab) * (py_a - py_b) / distance_ab**3 - 11*(-1/2+1/v_rel)*(vy_a - vy_b) / v_rel + 55.2*(vx_a - vx_b) / v_rel
...
def axis_accel(i, px1, py1, px2, py2, px3, py3, px4, py4, vx1_f, vy1_f, vx2_f, vy2_f, vx3_f, vy3_f, vx4_f, vy4_f):
        ...
        if i==0:
            ...
            aya =  -k_f * (py1 - py2 + 1.1) - k_f_v * (vy1_f - vy2_f) 
        elif i==1:
            axa =  -k_f * (-px1 + 2*px2 - px3 + 1.1) - k_f_v * (-vx1_f + 2*vx2_f - vx3_f) 
            aya =  -k_f * (-py1 + 2*py2 - py3 - 1.1) - k_f_v * (-vy1_f + 2*vy2_f - vy3_f) 
        elif i==2:
            axa =  -k_f * (-px2 + 2*px3 - px4 - 1.1) - k_f_v * (-vx2_f + 2*vx3_f - vx4_f) 
            aya =  -k_f * (-py2 + 2*py3 - py4 - 1.1) - k_f_v * (-vy2_f + 2*vy3_f - vy4_f)
        elif i==3:
            axa =  -k_f * (-px3 + px4 + 0) - k_f_v * (-vx3_f + vx4_f)
            aya =  -k_f * (-py3 + py4 + 1.1) - k_f_v * (-vy3_f + vy4_f) 
